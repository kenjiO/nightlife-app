...
module.exports = function(app, passport) {
	...
	app.use(function(req, res, next) {
		if (req.isAuthenticated()) {
			res.locals.username = req.user.github.username;
		}
		next();
	});
	
	// Set or load the location
	app.use(function(req, res, next) {
		var queryLocation = req.query.location;
		if (!req.user) {
			if (queryLocation)
				res.locals.location = queryLocation;
			return next();
		}

		var profileLocation = req.user.location;
		if (queryLocation && (queryLocation != profileLocation)) {
			//update saved location for user
			req.user.location = queryLocation;
			req.user.save(function(err, doc){
				if (err) return next(err);
			});
			res.locals.location = queryLocation;
		} else if (!queryLocation && profileLocation) {
			//load saved user location
			res.locals.location = profileLocation;
		}
		next();
	});
    ...
    app.get('/rsvp/:bar', function(req, res, next){
    	if (!res.locals.username) return res.redirect('/login');
    	Reservation.create({username: res.locals.username, bar: req.params.bar}, function(err, rsvp){
    		if (err) return next(err);
    		res.redirect('/');
    	});
    });
    
    app.get('/cancel/:bar', function(req, res, next){
    	if (!res.locals.username) return res.redirect('/login');
    	Reservation.remove({username: res.locals.username, bar: req.params.bar}, function(err){
    		if (err) return next(err);
    		res.redirect('/');
    	});
    });
	...
	app.route('/poll/:name')
		.get(function(req, res, next) {
			...
			Poll.findOne({
				'name': pollname
			}, 'name choices results', function(err, poll) {
				if (err) return next(err);
	...
	app.route('/edit/:name')
		.get(requireLoggedIn, function(req, res, next) {
			...
			Poll.findOne({
				'name': pollname
			}, 'owner name choices results', function(err, poll) {
				if (err) return next(err);
				if (!poll) return res.end("Poll " + pollname + " does not exist");
